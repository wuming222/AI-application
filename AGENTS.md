...
9. **dev 下新账号首挂载会自动建两条会话**：`Sidebar.tsx` 的挂载 effect 在 React `StrictMode` 下跑两遍，两遍都调 `initFirstSession()`，而它是"无会话才建"的判定 —— 两遍之间 store 还没落定，于是建出两条。`main` 上代码完全一致（`git show main:packages/web/src/components/Sidebar.tsx` 比对过），**不是本轮引入的回归，生产构建无双挂载**。浏览器实测时对"新账号应该只有一条会话"这类绝对数量断言要放宽成 `>= 1` + 相对增量断言，否则会把它当成 bug 去查。
