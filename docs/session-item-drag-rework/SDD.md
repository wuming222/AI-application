...
## 验收标准
- [ ] 会话项前没有图标，只有标题文本，超长仍是省略号
- [ ] 页面上不再存在独立拖拽手柄元素（`.drag-handle`）
- [ ] 按住整条会话项约 250ms 后上下移动，可以重排顺序
- [ ] 长按期间移动超过 5px 之前不会误触发拖拽
- [ ] 快速点击会话项只切换会话，不产生拖动
- [ ] 「⋯ → 重命名 / 删除」仍然可用；重命名输入框内可正常选中文本
- [ ] 排序结果仍持久化到 localStorage，刷新后顺序保持
- [ ] 拖动结束后卡片样式与位移在同一节点上，无残留错位
