...
- 结束后：最终回复单独成气泡，思考过程（`Message.reasoning`）在气泡内可展开回看。一次任务的多轮工具调用在 MessageList 渲染层合并为一个气泡：一级头部显式执行次数（"🔧 执行工具 N 次"，可折叠、默认展开），二级为每个工具的可折叠行，展开即该工具的执行过程（入参 + 结果，长文本截断展示）；工具气泡内不单独展示模型思考。store 消息结构保持 LLM 原始历史不变，分组只发生在渲染层
