...
2. `preview/buildSrcdoc.ts`：入口 HTML + 内联替换
...
6. 验证：生成应用 → 预览渲染 → 多轮迭代 → 中止

## 验收标准

- [ ] "创建一个待办事项应用" → Agent 调 write_file → 右侧 iframe 渲染出可交互页面
- [ ] 第二轮"把标题改成 X" → 文件更新 → 预览自动刷新
- [ ] 无 index.html 时不显示预览区
- [ ] function_call 轮次显示工具摘要而非 "..."
- [ ] tool 消息不重复出现在聊天列表
- [ ] 中止按钮正常，类型检查通过
