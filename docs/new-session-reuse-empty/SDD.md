...
## 验收标准
- [x] 停在空「新对话」上连点 3 次新建，会话总数不变（DOM 与 `GET /api/sessions` 都停在 6 条；改前同样的操作是 5 → 9）
- [x] 复用时光标落到输入框，输入框可用（实测 `document.activeElement` 为 composer TEXTAREA）
- [x] 当前会话已有消息时点新建，正常创建并切到新会话
- [ ] 当前会话只有生成文件、没有消息时点新建，正常创建 —— 本机没有这种数据态，未实测（代码按 `messageCount === 0 && fileCount === 0` 判定）
- [ ] 会话被重命名但仍无消息时仍复用 —— 同上，未实测；判定只看内容不看标题
- [x] 最后一条会话被删除、无当前会话时点新建，正常创建（`currentSessionId` 为空走新建分支）
- [x] 首屏自动建会话的行为不变（`createSession` 未改动，`initFirstSession` 仍走它）
- [x] 不新增任何后端请求：复用路径下 DB 会话数不变（等效验证；未逐条看 network 面板）
