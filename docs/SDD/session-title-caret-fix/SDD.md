...
## 验收标准
- [x] 单击/双击会话标题不产生文本选区，也不出现插入点
- [x] 标题元素默认态计算样式为 `user-select: none`（实测标题 div 与继承到的 svg 均为 `none`）
- [ ] 进入重命名后输入框可正常聚焦、选中、编辑、回车提交、Esc 取消 —— CSS 例外规则已加；in-app 浏览器视口隐藏导致无法用真实指针打开「⋯」菜单，未实测
- [x] 点击会话项仍能正常切换会话（实测轻点后顶栏标题切换）
