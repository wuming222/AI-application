...
仍未覆盖（别当已验证）：**语音这一路整体没有证据**。无头环境没麦克风，真机链路从没跑过；前端唯一的断言是 `authStore.test.ts` 里 `voiceWsUrl` 那两条（token 拼进 query 并做 URL 编码、未登录不拼出 `token=undefined`），麦克风采集与 PCM 编码零单测；服务端那一路的自行鉴权与 4401/4429 关闭码连断言都没有（`auth_check.py` 里一条 voice 都没有），只是读过代码。此前写成的"只做了单测层与关闭码翻译"偏乐观，按本条为准。另外打开 `SPEND_DAILY_LIMIT` 之后的**前端**表现、以及任何真实模型调用，同样未验证。
